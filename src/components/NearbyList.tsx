...
export function NearbyList({ trip, enabled, onItemClick }: Props) {
  ...
  const itemCount = sorted.filter(s => s.distance !== null).length;

  return (
    <div className="nearby-list">
      <div className="nearby-header">
        <span>{itemCount} places with location</span>
        <button className="nearby-refresh-btn" onClick={refresh}>
          Refresh Location
        </button>
      </div>
      {sorted.map(({ item, dayIndex, dayLabel, distanceLabel, distance }) => (
